...
export default function BlogManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing 
        ? `https://backend-car-rental-production.up.railway.app/api/blogs/${currentBlog._id}`
        : 'https://backend-car-rental-production.up.railway.app/api/blogs';
  ...
  const fetchBlogs = async () => {
    try {
      const response = await axios.get('https://backend-car-rental-production.up.railway.app/api/blogs');
      if (response.data.success) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get('https://backend-car-rental-production.up.railway.app/api/categories');
      if (response.data.success) {
  ...
  const handleEdit = (blog) => {
    ...
    setFormData({
      ...
      image: null,
      imagePreview: blog.image ? `https://api.cloudinary.com/v1_1/dlinqw87p/${blog.image}` : null,
      author: {
        name: blog.author?.name || '',
        image: null,
        imagePreview: blog.author?.image ? `https://api.cloudinary.com/v1_1/dlinqw87p/${blog.author.image}` : null
      }
    });
    setIsEditing(true);
    setShowEditor(true);
  ...
  const handleDelete = async (blogId) => {
    if (window.confirm('Are you sure you want to delete this blog?')) {
      try {
        await axios.delete(`https://backend-car-rental-production.up.railway.app/api/blogs/${blogId}`);
        fetchBlogs();
      } catch (error) {
